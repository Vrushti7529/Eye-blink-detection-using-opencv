...
while ret:
    ...
    if len(faces) > 0:
        for (x, y, w, h) in faces:
            ...
            if len(eyes) >= 2:
                if first_read:
                    cv2.putText(image, "Eyes are detected, press s to check blink", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (0, 255, 0), 2)
                else:
                    cv2.putText(image, "Eyes are Open", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (255, 255, 255), 2)
            else:
                if first_read:
